...
export class DatasheetVariablesService {
  ...
  async resolveVariable(name: string, params: any[], user: TenantPrincipal, bookId?: string): Promise<any> {
    ...
    try {
      switch (variable.nameEn) {
        ...
        case 'PRODUCT_COST': {
            const product = await this.productRepo.findOne({ where: { sku: params[0], organizationId: orgId } });
  ...
  async resolveBatch(variables: { name: string, params: any[] }[], user: TenantPrincipal) {
