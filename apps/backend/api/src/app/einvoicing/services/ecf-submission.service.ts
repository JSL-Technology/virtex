...
export class EcfSubmissionService {
  ...
  async submitInvoice(invoiceId: string, organizationId: string): Promise<EcfSubmission> {
    ...
    if (!invoice) throw new NotFoundError('einvoicing.invoice_invoice_id_not_found', { invoiceId });
    if (!invoice.isElectronicFiscalDocument) {
      throw new BadRequestError('einvoicing.invoice_has_no_electronic_ncf_assigned');
    }
  ...
  private async buildSignAndSend(
    ...
    if (!cert) {
      throw new BadRequestError('einvoicing.no_active_digital_certificate_sign_cf');
    }
