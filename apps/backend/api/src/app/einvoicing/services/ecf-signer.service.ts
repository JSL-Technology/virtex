...
export class EcfSignerService {
  ...
  sign(xml: string, cert: LoadedCertificate, rootLocalName: string): string {
    ...
    try {
      ...
      throw new InternalServerError('einvoicing.do.cf_document_could_not_signed_p1', { p1: (err as Error).message });
    }
  ...
  private extractSignatureValue(signedXml: string): string {
    ...
    if (!match) {
      throw new InternalServerError('einvoicing.do.signed_document_contains_no_signature_value');
    }
    return match[1];
  }
}
