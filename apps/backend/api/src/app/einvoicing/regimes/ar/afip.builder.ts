...
export class AfipBuilder {
   ...
   */
  documentType(input: Pick<AfipBuildInput, 'invoice' | 'customer' | 'issuerVatCondition'>): number {
  ...
  private assertIssuable(input: AfipBuildInput): void {
    if ((input.organization.taxId ?? '').replace(/\D/g, '').length !== 11) {
      throw new BadRequestError('einvoicing.ar.organization_has_no_valid_cuit_afip');
    }
    if (!input.salesPoint || input.salesPoint < 1) {
      throw new BadRequestError('einvoicing.ar.point_sale_afip_assigned_establishment_missing');
    }
    if (!input.nextNumber || input.nextNumber < 1) {
      throw new BadRequestError('einvoicing.ar.document_number_afip_expects_next_missing');
    }
  }
